...
public class ClientDaoImp implements IClientDao {
	...
	ClientTransactions transaction = new ClientTransactions();
	@Override
	...
	public double fundTransfer(BigInteger mobileNumber, double amount) throws ClientException {
		double balance = 0.0;
		if (ClientDb.getClientDb().get(0).getMobileNumber().equals(mobileNumber)) {
			if (ClientDb.getClientDb().get(0).getBalance() > amount) {
				balance = ClientDb.getClientDb().get(0).getBalance() - amount;
				ClientDb.getClientDb().get(0).setBalance(balance);
				double balance1 = ClientDb.getClientDb().get(1).getBalance() + amount;
	...
	@Override
	public ArrayList<ClientTransactions> printTransactions(LocalDateTime fromDate, LocalDateTime toDate, String type) {
		ArrayList<ClientTransactions> result = new ArrayList<>();
		for (ClientTransactions clientTran : result) {
			if (clientTran.gettDate().isAfter(fromDate) && clientTran.gettDate().isBefore(toDate)
